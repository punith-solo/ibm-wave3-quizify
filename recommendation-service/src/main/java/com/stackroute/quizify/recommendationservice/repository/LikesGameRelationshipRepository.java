...
public interface LikesGameRelationshipRepository extends Neo4jRepository<LikesGame, Long> {
    ...
    @Query("MATCH (p:User),(t:Game) WHERE id(p)={userId} and id(t)={gameId} CREATE (p)-[q:LikesGame}]->(t) RETURN p,q,t")
    LikesGame createRelationship(@Param("userId") long userId, @Param("gameId") long gameId);

    @Query("MATCH (p:User)-[r:LikesGame]->(t:Game) WHERE id(p)={userId} and id(t)={gameId} DELETE r RETURN 'relationship deleted' ")
