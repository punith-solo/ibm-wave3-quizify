...
public interface UserRepository extends Neo4jRepository<Users, Long> {
    ...
    @Query("MATCH (u:Users) WHERE u.id={userId} RETURN u")
    public Users getNode(@Param("userId") long userId);

    @Query("CREATE (u:Users) SET u.id={userId},u.name={userName},u.gender={gender} RETURN u")
    Users createNode(long userId, String userName, String gender);

    @Query("MATCH (n:Users) WHERE n.id={userId} DETACH DELETE n RETURN 'node deleted' ")
    Users deleteNode(@Param("userId") long userId);

    @Query("MATCH (n:Users) WHERE n.id={userId} SET n.userName={userName},n.gender={gender} RETURN n")
